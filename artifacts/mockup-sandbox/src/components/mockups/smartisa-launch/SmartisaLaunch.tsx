  ...
  return (
    <main className={`ceremony ${state} ${flash ? "flashed" : ""}`} aria-live="polite">
      <style>{`
        ...
        .finale .stage{animation:rise 1.2s both}.finale .eyebrow{color:var(--red)}.finale .title{font-size:clamp(74px,15vw,180px);font-weight:700;letter-spacing:.08em;color:var(--white);text-shadow:0 0 35px rgba(230,185,90,.3)}.finale .subtitle{color:var(--gold);font-family:'DM Sans',sans-serif;font-size:clamp(15px,2vw,22px);letter-spacing:.08em}.finale .final-line{font:10px 'Space Mono',monospace;letter-spacing:.28em;color:rgba(248,246,239,.5);margin-top:38px}.seal{width:54px;height:54px;border:1px solid var(--gold);border-radius:50%;display:grid;place-items:center;color:var(--gold);font:9px 'Space Mono',monospace;letter-spacing:.08em;margin-bottom:26px;position:relative}.seal:before{content:"";position:absolute;inset:5px;border:1px solid rgba(230,185,90,.35);border-radius:50%}
        canvas{position:absolute;inset:0;width:100%;height:100%;z-index:1;pointer-events:none}.countdown .launch-wrap{display:none}
        @media(max-width:600px){.topline{top:18px;font-size:8px}.topline .mark{font-size:9px}.replay{padding:8px 9px;font-size:8px}.subtitle{letter-spacing:.17em;margin-bottom:32px}.hint{font-size:8px;letter-spacing:.1em}.finale .title{font-size:22vw;letter-spacing:.04em}}
      `}</style>
      ...
      <section className="stage">
        ...
        {state === "finale" && <><div className="seal">81<br />RI</div><div className="eyebrow">Resmi Diluncurkan</div><h1 className="title">SMARTISA</h1><p className="subtitle">Solusi Cerdas untuk Indonesia Maju</p><div className="final-line">DIRGAHAYU REPUBLIK INDONESIA &nbsp;·&nbsp; 17 AGUSTUS 2026</div></>}
      </section>
    </main>
  );
}